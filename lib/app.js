...
app.get('/snowboards', async(req, res) => {
  try {
    ...
    select snowboards.id, snowboards.snowboard_name, snowboards.flex, snowboards.is_all_mountain, brands.name as brand, snowboards.owner_id
      from snowboards
      join brands
      on brands.id = snowboards.brand_id
...
app.get('/snowboards/:id', async(req, res) => {
  try {
    ...
    const data = await client.query(`
      select
        snowboards.id,
        snowboards.snowboard_name,
        snowboards.flex,
        snowboards.is_all_mountain,
...
app.post('/snowboards', async(req, res) => {
  try {
    ...
    VALUES ($1, $2, $3, $4, $5)
    RETURNING *
  `,  [newName, newFlex, newIsAllMountain, newBrandId, newOwnerId]);
...
app.put('/snowboards/:id', async(req, res) => {
  try {
    ...
    RETURNING *
  `,  [newName, newFlex, newIsAllMountain, newBrandId, newOwnerId, req.params.id]);
